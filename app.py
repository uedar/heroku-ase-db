...
def update(sid: int, what: str, x: str):
    ...
    * page: show page x
    ...
    return render_template(project["table_template"], t=table, p=project, s=session)
...
def row(project_name: str, uid: str):
    ...
    row = project["database"].get("{uid_key}={uid}".format(uid_key=uid_key, uid=uid))
    dct = project["row_to_dict_function"](row, project)
    return render_template(project["row_template"], d=dct, row=row, p=project, uid=uid)
